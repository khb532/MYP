...
에이전트는 현재 작업 경로에서 가장 가까운 AGENTS.md를 로드하며, 하위 디렉터리에 별도 문서가 없다면 상위 디렉터리의 지침을 그대로 따릅니다. 문서가 여럿 존재하면 가장 가까운 문서가 우선하며, 해당 문서에서 명시된 항목만 상위 지침을 덮어씁니다.

## 구성 방향
- 문서 목적을 한 줄 요약으로 시작하고, 시스템 → 개발자 → AGENTS 순의 우선순위와 적용 범위를 명확히 기술합니다.
- Unreal Engine 5 + C++ 프로젝트 특성을 반영해 작업에 꼭 필요한 규칙 위주로 정리하고, 세부 참고 자료는 별도 문서나 링크로 분리합니다.
- 문서 유지보수를 고려해 변경 시점과 책임자, 검토 주기를 간단히 남깁니다.

## 주요 섹션
- 프로젝트 개요: 엔진 버전, <PROJECT_NAME>, <MODULE_NAME>, 경로 변수 같은 공통 정의.
- 디렉터리 배치: Source/<MODULE_NAME>/Public|Private/<Domain> 구조, 신규 파일 위치 규칙, 예외 조건.
- 코딩 스타일: 네이밍 접두사, 매크로/로그 사용, 헤더 포함 원칙 등 필수 컨벤션.
- 편집 제한: 수정 금지 파일, 생성물 커밋 금지, 기본 인코딩(ASCII) 등.
- 빌드·테스트: Rider 설정, 커맨드라인 빌드, Automation Test 실행 예시.
- 커밋·PR: 제목 태그 형식, 본문 작성 요령, 이슈 연계 방식.
- 보안·비밀 관리: 민감 정보 처리, 설정 파일 관리 지침.
- 커뮤니케이션: 응답 언어(한국어), 톤, 결과 보고 형태.

## 운영 팁
- 각 항목은 “필수 규칙 → 예시 또는 금지/권장” 순으로 짧게 정리해 가독성을 높입니다.
- 중복·모호한 표현을 피하고, 자주 사용하는 명령이나 경로는 예시로 제시해 실행 속도를 높입니다.
- 하위 디렉터리에 별도 AGENTS.md가 필요하면 상위 문서와 달라지는 최소 항목만 명시해 충돌을 줄입니다.
- 프로젝트 구조나 규칙이 바뀌면 즉시 문서를 갱신해 실제 상태와 지침 사이의 괴리를 막습니다.
